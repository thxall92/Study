/*
  ...
  1. 웹소켓 연결
  2. 오더북 불러옴
  3. 처음에는 전체 오더북 데이터를 가져옴
  4. Book이라는 변수에 원하는 모양으로 가공해서 전체 오더북 데이터를 저장
  5. 순서대로 정리해줌
  6. 이후로 업데이트 되는 데이터 값을 받으면 실시간으로 업데이트를 해줌
  7. 또 순서대로 정리해줌
  8. 값이 제대로 업데이트 되게끔 더블체크를 해줌
  9. 이 오더북 데이터를 5분마다 log 파일에 저장함
...
function connect() {
  ...
  cli.on('message', function(msg) {
    ...
    // 6. 위에서 받은 데이터를 BOOK.psnap 변수에다가 순서대로 정리해주는 단계
